...
- `api/service` gains a display service: authenticate, resolve the public UUID to an internal entity
  ID, call `Registry.Render` for `display.FieldName`, and map `ErrRendererNotRegistered` to a
  non-error "unavailable" outcome. Plus a constructor that builds a registry with mod-core's
...
- The same route returns `200` with `{"uuid": "...", "display_name": null}` — never a 4xx or 5xx —
  when no renderer is registered for that entity's type, when no registry is wired into the
  deployment at all, and when the UUID names no entity. This is the expected steady state for an
  entity type owned by a module absent from a given deployment.
...
- Any change to **mod-workflows** — its GUI still needs a client call once a composed app exists.
  mod-workflows followup `98Bq` is **not** closed by this plan.
...
- `api/authz/authz.go` exports `RequireAuthenticated`; it has no production call site yet.
- `api/entity/resolver.go`'s sentinels are aliased to the `apiresp` sentinels, so a masked miss
  already classifies correctly through `apiresp.WriteError`.
...
1. **`001-add-display-service`** *(sonnet-high)* — `api/service/display.go`: a
   ...
   authenticates via `authz.RequireAuthenticated`, resolves the UUID through the shared
   `entity.Resolver`, calls `Render`, and reports `ErrRendererNotRegistered` (and an unknown UUID)
   as a non-error "unavailable" result. Unit tests cover resolved, not-registered, unknown-UUID,
   unauthenticated, and genuine-render-error paths.
...
- [Display HTTP surface design](./notes/display-http-surface-design.md) — endpoint and response
  shape, error mapping, and the authentication-only authorization decision with its precedent.
