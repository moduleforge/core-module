...
| `display/` | Per-`(typeSlug, fieldName)` renderer registry. Modules register renderers for their own entity kinds; the registry dispatches at render time. A composing app obtains the single shared registry via `coreservice.NewDisplayRegistry`, which also registers mod-core's own builtin renderers; the module manifest provides it as the `displayRegistry` service. |
